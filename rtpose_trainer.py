...
def main():
    ...
    cocodset = CocoPoseDataset(os.path.join(base_path, 'images'), os.path.join(base_path, 'annotations'), transforms=trans)
    ...
    for e in range(epochs):
        for i, data in enumerate(cocodset):
            ...
            if i % 100 == 0:
                print('Epoch [%d/%d], Batch [%d/%d], Total Loss %f' % (e, epochs, i, len(cocodset), curr_loss.item()))
